    ...
    <div id="contentwrapper" class="contentwrapper">
        <div id="validation" class="subcontent">
        	<div class="contenttitle2">
                ...
                <form id="form1" class="stdform" method="post" action="<?php echo U('Admin/Student/update');?>">
                <input type='hidden' value='<?php echo ($id); ?>' name = 'id'/>
                	<p>
                    	<label>姓名</label>
                        <span class="field"><input type="text" name="name" id="firstname" class="longinput" value='<?php echo ($edit_stu["name"]); ?>'/></span>
                    </p>
                    
                    <p>
                    	<label>学号</label>
                        <span class="field"><input type="text" name="number_id" id="number" class="longinput" value='<?php echo ($edit_stu["number_id"]); ?>'/></span>
                    </p>
                    
                    <p>
                    	<label>性别</label>
                        <span class="formwrapper">
                        	<input type="radio" name="sex"  value = '0' <?php if($edit_stu["sex"] == 0): ?>checked<?php endif; ?>/>男
                        	<input type="radio" name="sex" value = '1'  <?php if($edit_stu["sex"] == 1): ?>checked<?php endif; ?>/>女
                        </span>
                    </p>
                    <p>
                    	<label>校区</label>
                        ...
                        <select name="school_id" id="selection" class='school'>
                            <?php if(is_array($school)): foreach($school as $key=>$data): ?><option value='<?php echo ($data["id"]); ?>' <?php if($edit_stu["school_id"] == $data['id']): ?>selected='selected'<?php endif; ?>><?php echo ($data['names']); ?></option><?php endforeach; endif; ?>
                        </select>
                        </span>
                    </p>
                   <p>
                        ...
                        <select name="class_id" id="selection" class='classes'>
                        	...
                        	<?php if(is_array($class)): foreach($class as $key=>$row): ?><option value='<?php echo ($row["id"]); ?>'<?php if($edit_stu["class_id"] == $row['id']): ?>selected<?php endif; ?>><?php echo ($row['name']); ?></option><?php endforeach; endif; ?>
                        </select>
                        </span>
                    </p>
                    <p>
