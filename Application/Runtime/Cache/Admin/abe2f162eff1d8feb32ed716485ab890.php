    ...
    <div class="centercontent">
        ...
        <div id="contentwrapper" class="contentwrapper">
        	...
        	<div id="updates" class="subcontent">
                ...
                <tbody>
                <?php if(is_array($main)): foreach($main as $key=>$class): ?><tr tabindex='-1' style="outline:none" >
                    	<td align="center"><input type="checkbox" /></td>
                        <td><?php echo ($num++); ?></td>
                        <td><?php echo ($class['name']); ?></td>
                        <td style="display:table-cell;"><?php echo ($class['names']); ?></td>
                        <td style="display:none;">
                        	<select name='school_id' class="edit_select">
                       		<?php if(is_array($data)): foreach($data as $key=>$classname): ?><option value='<?php echo ($classname['id']); ?>' <?php if($class['names'] == $classname['names']): ?>selected="selected"<?php endif; ?> ><?php echo ($classname['names']); ?></option><?php endforeach; endif; ?>
                        	</select>
                        </td>
                        <td class='center'>
                        	<input type="hidden" value="<?php echo ($class['id']); ?>" name="id" class="edit_id">
                           <!--<?php echo U('Admin/Student/edit',array('id'=>$data['id']));?> <a>原标签中传值-->
                            ...
                            <a href="<?php echo U('Admin/Other/deleteclass',array('id'=>$class['id']));?>" class="btn btn_trash"><span class="dels">删除</span></a>
            ...
            <div id="activities" class="subcontent" style="display:none;" >
                <form id="form1" class="stdform" method="post" action="<?php echo U('Admin/Other/Addclass');?>">
                    <p>
                        <label>校区名称</label>
                        <select class="radius3" name='school_id'>
                            <!-- <option value="">请选择校区</option> -->
                            <?php if(is_array($data)): foreach($data as $key=>$classname): ?><option value="<?php echo ($classname['id']); ?>"><?php echo ($classname['names']); ?></option><?php endforeach; endif; ?>
                        </select> &nbsp;
                    </p>
                	<p>
                    	<label>班期名称</label>
...
*/
			...
			$(".edits").click(function(){
		        ...
		        //编写html添加进去。
		        var html = "<input type='text' name='username'  placeholder='"+oldText1+"' class = 'edit_input'/> ";
		       ...
		       // 确认修改按钮的事件
		        $('.sure_edit').click(function(){
			        ...
			        if(newText1==''){
			        	...
			        	$.post('<?php echo U('Admin/Other/editclass');?>',{'edit_id':edit_id,'input_value':newText1,'select_id':newText2},function(data){
		                    if(data == 1){
